...
// Seamless highlight control
let matchPageSet = new Set();  // pages that have matches
...
loadAllBtn.addEventListener('click', async () => {
  ...
  // If we had a search active, highlight any newly loaded match pages:
...
async function autoRenderInitialPages() {
  ...
  if (toLoad < total) {
    setStatus(`Preview loaded (${toLoad}/${total}). Load All Pages or search to jump to matches.`);
  }
}
...
async function runSearch() {
  ...
  try {
    ...
    setStatus(`Found ${searchResults.length} page(s). Preparing highlights...`);

    // Build set of match pages
    matchPageSet = new Set(searchResults.map(r => r.page));

    // Preload only MATCH pages (seamless)
    await preloadMatchPagesWithProgress();

    // Highlight all match pages (ones loaded)
    seamlessHighlightActive = true;
    highlightAllMatchPages();

    // Select first page (scroll + text) but DO NOT clear other highlights
    selectResultIndex(0, {preserveHighlights:true});

    setStatus(`Highlights ready across ${searchResults.length} page(s). Scroll freely.`);
  } catch (err) {
    console.error(err);
...
async function selectResultIndex(idx, opts = {}) {
  ...
  // If seamless mode is active, we DO NOT clear other highlights
  if (seamlessHighlightActive) {
...
async function ensurePageLoaded(pageNum) {
  ...
  try {
    ...
    // If seamless highlight active and this page is a match page, highlight it now.
...
function highlightPageMatches(pageNum, {append=false} = {}) {
  ...
  // Only highlight tokens whose lowercased text is in targets
...
function highlightAllMatchPages() {
  for (const pageNum of matchPageSet) {
    if (pageCache[pageNum]) {
      // Clear just that page then reapply (ensures fresh sizing)
...
// Recompute highlights on window resize (image width changes)
window.addEventListener('resize', () => {
  if (seamlessHighlightActive) {
    // Recreate highlights for all match pages
    highlightAllMatchPages();
...
async function applyZoom(newScale) {
  ...
  // Highlights scale automatically with transform.
  // If you wanted pixel-perfect regen after zoom, uncomment:
