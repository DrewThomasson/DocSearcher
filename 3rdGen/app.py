...
# OCR configuration
OCR_DESKEW = True
OCR_OPTIMIZE = 3                # 0..3 (3 strongest optimization)
OCR_SKIP_TEXT = True            # Skip pages that already have text
OCR_MAX_PAGES = 5000
OCR_TIMEOUT_SECONDS = 1800      # 30 min
# ALWAYS ENABLE AUTO ROTATION:
OCR_ROTATE_PAGES = True
OCR_ROTATE_PAGES_THRESHOLD = 1.0  # 0–100 (higher = more conservative)

# Include stack traces inside ocr_message when failures occur (disable for prod)
DEBUG_OCR_ERRORS = True

app = Flask(__name__)
app.config['MAX_CONTENT_LENGTH'] = MAX_FILE_SIZE_MB * 1024 * 1024  # Hard cap at Flask level
...
def perform_ocr(original_path: str, doc_id: str, lang: str):
    """
    Always performs OCR with auto-rotation.
    Returns:
      active_pdf_path, ocr_performed, ocr_failed, ocr_message, ocr_pdf_path, ocr_time_seconds
    """
    try:
